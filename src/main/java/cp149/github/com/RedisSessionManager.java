...
public class RedisSessionManager extends ManagerBase implements Lifecycle {
	...
	protected String serializationStrategyClass = "cp149.github.com.JavaSerializer";
	...
	public RedisSession loadSessionFromRedis(String id) throws IOException {
		...
		try {
			...
			if (data == null) {
				...
				session = null;
				// throw new
				// IllegalStateException("Race condition encountered: attempted to load session["
	...
	private void initializeDatabaseConnection() throws LifecycleException {
		try {
			...
			s.add(getHost() + ":" + getPort());
			if (masterName == null)
				connectionPool = new JedisPool(poolConfig, host, port, timeout, password);
			else
				connectionPool = new JedisSentinelPool(masterName, s, poolConfig, getTimeout(), getPassword());
		} catch (Exception e) {
			e.printStackTrace();
